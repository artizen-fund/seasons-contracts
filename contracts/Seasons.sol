    ...
    ERC1155URIStorageUpgradeable
{
    ...
    uint[] private amountsBoughtPerAddress;
    ...
    event ArtifactsClaimedByArtist(
        address artist,
        uint tokenID,
        uint amount,
        uint timestamp
    ...
    */
    ...
    function mintArtifact(
        ...
        if (seasons[seasonOfSubmission].endTime < block.timestamp)
            revert SeasonAlreadyClosed(seasonOfSubmission);
        if (
            isBlacklistedInSeason[submissions[tokenIDToMint].season][
                tokenIDToMint
            ]
        )
    ...
    function artistClaim(
        ...
        if (msg.sender != submissions[submissionID].SubmissionOwner)
            revert OnlyArtist();
        if (
            isBlacklistedInSeason[submissions[submissionID].season][
                submissionID
            ]
        )
            revert ProjectBlacklistedInSeason(
        ...
        // TODO - add viewn function for balances
    ...
    */
    ...
    function blacklistSubmissionFromSeason(
        ...
        amountSoldBeforeBlackList[seasonID][
            submissionID
        ] = totalAmountOfTokensSold[submissionID];
